...
@Transactional
	...
	public void create(MemberVO member) throws Exception {
		...
		dao.createAuthKey(member.getFullemail(), key); // 인증키 DB저장

		MailHandler sendMail = new MailHandler(mailSender);
		sendMail.setSubject("[회원가입 인증 코드 발급 안내입니다.]");
		/*sendMail.setText(
				new StringBuffer().append("<h1>이메일 인증 확인을 누르시고 로그인 해주세요.</h1>").append("<a href='http://192.168.56.1:8080/membership/emailConfirm?email=").append(member.getFullemail()).append("' target='_blank'>이메일 인증 확인</a>").toString());
		*/
		sendMail.setText(

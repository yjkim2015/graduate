...
.visual{width:100%; min-height:400px; overflow:hidden; padding:80px 0 0 0; background-image:url("../resources/assets/images/help1.jpg"); background-size:100% 100%;}
...
$(document).ready(function(){
	...
	$("#modifyBtn").on("click", function(){
		formObj.attr("action", "/help/modifyPage");
		...
		console.log(arr);
	 	if(arr.length > 0){
			$.post("/deleteAllFiles",{files:arr}, function(){
				...
				formObj.attr("action", "/help/removePage");
				formObj.submit();
...
<form role="form" method="post" action="modifyPage">
	<input type="hidden" name="bno" value="${boardVO4.bno }">
...
<div class="box-body" style="padding:50px">
	<div class="form-group">
		...
		name="title" class="form-control" value="${boardVO4.title }" style="background-color:white; font-size:15px;"
		 readonly="readonly" >
	</div>
	<div class="form-group">
		<label for="exampleInputPassword1" style="font-size:20px">내용</label>
		<textarea class="form-control" name="content" rows="3" readonly="readonly" style="background-color:white; font-size:15px;">${boardVO4.content}</textarea>
	</div>
	<div class="form-group">
		<label for="exampleInputEmail1"style="font-size:20px">글쓴이</label>
		<input type="text" name="writer" class="form-control" value="${boardVO4.writer }" style="background-color:white; font-size:15px;"
		readonly="readonly">
	</div>
</div>
...
<!-- 		<strong class="title">산타마리아호</strong> -->
    ...
    <br>
       ...
       <script>
    if("${boardVO4.writer}"=="${name}"){
	document.write("<button type='submit' class='btn btn-warning' id='modifyBtn' style='margin-left:40px;'>수정</button>");
...
<ul class="timeline" style="margin-left:50px;" >
	<li class="time-label" id="repliesDiv"><span class="bg-green">
	 댓글 목록<small id="replycntSmall">[${boardVO4.replycnt }]</small></span></li>
</ul>
...
$("#repliesDiv").on("click",function(){
	...
	getPage("/replies4/"+bno+"/1");
});

$(".pagination").on("click","li a",function(event){
	event.preventDefault();
	replyPage =$(this).attr("href");
	getPage("/replies4/"+bno+"/"+replyPage);
});
...
$.getJSON("/help/getAttach/"+bno,function(list){
	$(list).each(function(){
